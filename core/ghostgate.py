...
async def create_subscription(comment, data_gb, days, ip_limit, node_ids):
    r = await _post("subscriptions", json={"comment": comment, "data_gb": data_gb, "days": days, "ip_limit": ip_limit, "node_ids": node_ids})
    return r.json() if r else None
