...
class Updater:
    def __init__(self, check_interval=300, check_on_startup=True, http_proxy="", https_proxy=""):
        ...
        self.update_url=f"https://github.com/{GITHUB_REPO}/releases/latest/download/ghostpass"
    ...
    async def download_update(self, new_version):
        try:
            ...
            tmpdir=f"/tmp/ghostpass-update-{os.getpid()}"
            os.makedirs(tmpdir, exist_ok=True)
            binary_path=os.path.join(tmpdir, "ghostpass")
    ...
    async def manual_update(self):
        ...
        ret=subprocess.run(["systemctl", "restart", "ghostpass"], capture_output=True)
        if ret.returncode==0:
            print("Service ghostpass restarted.")
        else:
            print("Update installed. Restart manually: systemctl restart ghostpass")
